...
function DrawParkCell(blockX, blockZ, cellX, cellZ, cellW, cellL) {
    ...
    // reservoir's walls
    FillCellLayer(BlockID.NETHER_BRICK, blockX, blockZ, 0, cellW, cellL);
    AddWalls(BlockID.NETHER_BRICK, blockX, 1, blockZ,
        ...
        for (var z = cornerWidth; z < cellLength; z = z + cornerWidth)
            for (var y = 1; y < streetLevel; y++) {
                ...
                if (y == 1 && ((x % 2 == 0 && z % 2 == 1) ||
                    ...
                    blocks[blockX + x][y][blockZ + z] = BlockID.NETHER_BRICK;
            }

    // cap it off
    FillCellLayer(BlockID.NETHER_BRICK, blockX, blockZ, streetLevel, cellW, cellL);

    // add some grass
    FillCellLayer(BlockID.GRASS, blockX, blockZ, streetLevel + 1, cellW, cellL);

    // steps up to access point
    blocks[blockX + 7][streetLevel - 6][blockZ + 1] = BlockID.NETHER_BRICK;
    blocks[blockX + 6][streetLevel - 5][blockZ + 1] = BlockID.NETHER_BRICK;
    blocks[blockX + 5][streetLevel - 4][blockZ + 1] = BlockID.NETHER_BRICK;

    // platform for access point
    blocks[blockX + 4][streetLevel - 3][blockZ + 1] = BlockID.NETHER_BRICK;
    blocks[blockX + 3][streetLevel - 3][blockZ + 1] = BlockID.NETHER_BRICK;
    blocks[blockX + 2][streetLevel - 3][blockZ + 1] = BlockID.NETHER_BRICK;
    blocks[blockX + 1][streetLevel - 3][blockZ + 1] = BlockID.NETHER_BRICK;

    // backfill the wall behind the ladders
    blocks[blockX + 2][streetLevel - 2][blockZ + 1] = BlockID.NETHER_BRICK;
    blocks[blockX + 1][streetLevel - 2][blockZ + 1] = BlockID.NETHER_BRICK;
    blocks[blockX + 2][streetLevel - 1][blockZ + 1] = BlockID.NETHER_BRICK;
    blocks[blockX + 1][streetLevel - 1][blockZ + 1] = BlockID.NETHER_BRICK;
    ...
    for (var x = 0; x < cellWidth; x++) {
        ...
        if (x == fenceHole || x == cellWidth - fenceHole - 1) {
            blocks[blockX + x][streetLevel + 2][blockZ] = BlockID.NETHER_BRICK;
            blocks[blockX + x][streetLevel + 2][blockZ + cellLength - 1] = BlockID.NETHER_BRICK;
            blocks[blockX + x][streetLevel + 3][blockZ] = BlockID.NETHER_BRICK;
            blocks[blockX + x][streetLevel + 3][blockZ + cellLength - 1] = BlockID.NETHER_BRICK;
            ...
            blocks[blockX + x][streetLevel + 2][blockZ] = BlockID.NETHER_BRICK_FENCE ;
            blocks[blockX + x][streetLevel + 2][blockZ + cellLength - 1] = BlockID.NETHER_BRICK_FENCE ;
        }
    }
    ...
    for (var z = 0; z < cellLength; z++) {
        if (z == fenceHole || z == cellLength - fenceHole - 1) {
            blocks[blockX][streetLevel + 2][blockZ + z] = BlockID.NETHER_BRICK;
            blocks[blockX + cellWidth - 1][streetLevel + 2][blockZ + z] = BlockID.NETHER_BRICK;
            blocks[blockX][streetLevel + 3][blockZ + z] = BlockID.NETHER_BRICK;
            blocks[blockX + cellWidth - 1][streetLevel + 3][blockZ + z] = BlockID.NETHER_BRICK;
            ...
            blocks[blockX][streetLevel + 2][blockZ + z] = BlockID.NETHER_BRICK_FENCE ;
            blocks[blockX + cellWidth - 1][streetLevel + 2][blockZ + z] = BlockID.NETHER_BRICK_FENCE ;
        }
    }
...
function AddCitySquares() {
    ...
    function DrawBuildingCell(blockX, blockZ, cellX, cellZ, cellW, cellL) {
        ...
        if (roofStyle == 3) {
            ...
            AddWalls(BlockID.NETHER_BRICK_FENCE , blockX + 1, roofAt + floorHeight, blockZ + 1,
        ...
        function RandomMaterial() {
            switch (rand.nextInt(13)) {
                ...
                case 6: return BlockID.NETHER_BRICK;
        ...
        function PunchBasementStairs(atX, atZ, basements, morestairs) {
            if (basements > 0) {
                ...
                AddWalls(BlockID.NETHER_BRICK_FENCE , atX - 1, streetLevel + 2, atZ - 2,
                                        atX + 2, atY + 1, atZ + 2);
        ...
        function PunchStairs(stairID, atX, atZ, floorY, addStairwell, addGuardrail, drawStairs) {
            ...
            for (airX = atX; airX < atX + stairHeight; airX++) {
                ...
                if (addGuardrail) {
                    blocks[airX][floorY + 1][atZ + 2] = BlockID.NETHER_BRICK_FENCE ;
                    blocks[airX][floorY + 1][atZ - 1] = BlockID.NETHER_BRICK_FENCE ;
                }
            }
        ...
        function FinalizeStairs(atX, atZ, floorAt, addGuardrail) {
            if (addGuardrail) {
                ...
                for (var z = 0; z < 4; z++)
                    blocks[atX - 2][floorAt][atZ + z - 2] = BlockID.NETHER_BRICK_FENCE ;
            }
        }
    }
}
...
function AddStreets() {
    ...
    function DrawStreetCell(blockX, blockZ, cellX, cellZ) {
        ...
        // add the street
        FillCellLayer(BlockID.NETHER_BRICK, blockX, blockZ, streetLevel - 1, 1, 1);
        FillCellLayer(BlockID.NETHER_BRICK, blockX, blockZ, streetLevel, 1, 1);
        ...
        function DrawSewerPart(atX, atZ, boxX, boxZ) {
            ...
            var doorID = BlockID.NETHER_BRICK;
...
function DrawStreetlight(blockX, blockY, blockZ, lightN, lightE, lightS, lightW) {
    blocks[blockX][blockY][blockZ] = BlockID.NETHER_BRICK;
    blocks[blockX][blockY + 1][blockZ] = BlockID.NETHER_BRICK_FENCE ;
    blocks[blockX][blockY + 2][blockZ] = BlockID.NETHER_BRICK_FENCE ;
    blocks[blockX][blockY + 3][blockZ] = BlockID.NETHER_BRICK_FENCE ;
    blocks[blockX][blockY + 4][blockZ] = BlockID.NETHER_BRICK_FENCE ;
...
function AddFarmAndHousesLot() {
    ...
    for (var at = 0; at < squareBlocks * 3; at++) {
        ...
        if (at < lotBlocks - 2 || at > lotBlocks + 2) {
            blocks[squareBlocks + at][streetLevel + 2][squareBlocks + lotBlocks] = BlockID.NETHER_BRICK_FENCE ;
            blocks[squareBlocks + lotBlocks][streetLevel + 2][squareBlocks + at] = BlockID.NETHER_BRICK_FENCE ;
        }
    }
...
function AddParkingLot() {
    ...
    var floorID = BlockID.NETHER_BRICK;
    var ceilingID = BlockID.NETHER_BRICK;
    ...
    var fenceID = BlockID.NETHER_BRICK;
    ...
    switch (rand.nextInt(3)) {
        case 0:
            {
                fenceID = BlockID.NETHER_BRICK_FENCE ;
    ...
    if (OneInTwoChance() && fenceID != BlockID.NETHER_BRICK_FENCE )

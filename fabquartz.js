...
// screenshot https://minecraftgods.com/?p=118  difference, left default cloth, changed fence ends.
// $Id$
/***** FAB.JS v1.0
...
function DrawParkCell(blockX, blockZ, cellX, cellZ, cellW, cellL) {
    ...
    // cap it off
    FillCellLayer(BlockID.QUARTZ_BLOCK, blockX, blockZ, streetLevel, cellW, cellL);

    // add some grass
    FillCellLayer(BlockID.GRASS, blockX, blockZ, streetLevel + 1, cellW, cellL);

    // steps up to access point
    blocks[blockX + 7][streetLevel - 6][blockZ + 1] = BlockID.QUARTZ_BLOCK;
    blocks[blockX + 6][streetLevel - 5][blockZ + 1] = BlockID.QUARTZ_BLOCK;
    blocks[blockX + 5][streetLevel - 4][blockZ + 1] = BlockID.QUARTZ_BLOCK;

    // platform for access point
    blocks[blockX + 4][streetLevel - 3][blockZ + 1] = BlockID.QUARTZ_BLOCK;
    blocks[blockX + 3][streetLevel - 3][blockZ + 1] = BlockID.QUARTZ_BLOCK;
    blocks[blockX + 2][streetLevel - 3][blockZ + 1] = BlockID.QUARTZ_BLOCK;
    blocks[blockX + 1][streetLevel - 3][blockZ + 1] = BlockID.QUARTZ_BLOCK;

    // backfill the wall behind the ladders
    blocks[blockX + 2][streetLevel - 2][blockZ + 1] = BlockID.QUARTZ_BLOCK;
    blocks[blockX + 1][streetLevel - 2][blockZ + 1] = BlockID.QUARTZ_BLOCK;
    blocks[blockX + 2][streetLevel - 1][blockZ + 1] = BlockID.QUARTZ_BLOCK;
    blocks[blockX + 1][streetLevel - 1][blockZ + 1] = BlockID.QUARTZ_BLOCK;
    ...
    for (var x = 0; x < cellWidth; x++) {
        ...
        if (x == fenceHole || x == cellWidth - fenceHole - 1) {
            blocks[blockX + x][streetLevel + 2][blockZ] = BlockID.QUARTZ_BLOCK;
            blocks[blockX + x][streetLevel + 2][blockZ + cellLength - 1] = BlockID.QUARTZ_BLOCK;
            blocks[blockX + x][streetLevel + 3][blockZ] = BlockID.QUARTZ_BLOCK;
            blocks[blockX + x][streetLevel + 3][blockZ + cellLength - 1] = BlockID.QUARTZ_BLOCK;
    ...
    for (var z = 0; z < cellLength; z++) {
        if (z == fenceHole || z == cellLength - fenceHole - 1) {
            blocks[blockX][streetLevel + 2][blockZ + z] = BlockID.QUARTZ_BLOCK;
            blocks[blockX + cellWidth - 1][streetLevel + 2][blockZ + z] = BlockID.QUARTZ_BLOCK;
            blocks[blockX][streetLevel + 3][blockZ + z] = BlockID.QUARTZ_BLOCK;
            blocks[blockX + cellWidth - 1][streetLevel + 3][blockZ + z] = BlockID.QUARTZ_BLOCK;
...
function AddStreets() {
    ...
    function DrawStreetCell(blockX, blockZ, cellX, cellZ) {
        ...
        // add the street
        FillCellLayer(BlockID.QUARTZ_BLOCK, blockX, blockZ, streetLevel - 1, 1, 1);
        FillCellLayer(BlockID.QUARTZ_BLOCK, blockX, blockZ, streetLevel, 1, 1);
...
function DrawStreetlight(blockX, blockY, blockZ, lightN, lightE, lightS, lightW) {
    blocks[blockX][blockY][blockZ] = BlockID.QUARTZ_BLOCK;
...
function AddParkingLot() {
    ...
    var floorID = BlockID.QUARTZ_BLOCK;
